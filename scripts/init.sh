...
tmux send-keys -t 'docker-asp' 'cd ~/Sources/chat/docker/' C-m

tmux new-window -t $session:3 -n 'docker-wcf'
tmux send-keys -t 'docker-wcf' 'cd ~/Sources/chat/docker/' C-m

tmux new-window -t $session:4 -n 'docker-wcf-bin'
tmux send-keys -t 'docker-wcf-bin' 'cd ~/Sources/chat/docker/' C-m

tmux new-window -t $session:5 -n 'docker-asp-bin'
tmux send-keys -t 'docker-asp-bin' 'cd ~/Sources/chat/docker/' C-m
